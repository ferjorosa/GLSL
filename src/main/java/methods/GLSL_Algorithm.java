...
*
* */
public class GLSL_Algorithm implements LatentDiscreteMethod, LatentContinuousMethod, LatentMixedMethod, BayesianMethod, MissingDiscreteMethod, MissingContinuousMethod, MissingMixedMethod {
    ...
    private Tuple2<BayesianNetwork, Double> initialize(DataOnMemory<DataInstance> data,
        ...
        switch (this.initialization) {
            ...
            case CIL_1:
                Tuple3<BayesianNetwork, Double, Long> result_cil_1 = ConstrainedIncrementalLearner.learnModel(data,
                        priors,
                        seed,
                        1,
                        true,
            ...
            case CIL_10:
                Tuple3<BayesianNetwork, Double, Long> result_cil_10 = ConstrainedIncrementalLearner.learnModel(data,
                        priors,
                        seed,
                        10,
                        true,
...
//                + "_folds/GLSL", dataName, "GLSL");
    ...
    public void runMissingContinuous(Tuple2<DataOnMemory<DataInstance>, List<DataOnMemory<DataInstance>>> runs,
        ...
        /* Iterate through the runs and learn a model for each one */
        ...
        for(int i = 0; i < missingDataSets.size(); i++) {
            ...
            /* Initial model */
            Tuple2<BayesianNetwork, Double> initialResult = initialize(missingData, vbemConfig);

            /* Learn model using GLSL */
            long initTime = System.currentTimeMillis();
            Tuple2<BayesianNetwork, Double> glsl_result = glsl.learnModel(initialResult.getFirst(), initialResult.getSecond(), missingData, LogUtils.LogLevel.INFO, LogUtils.LogLevel.INFO);
            BayesianNetwork posteriorPredictive = glsl_result.getFirst();
            long endTime = System.currentTimeMillis();
        ...
        /* Store models */
        if(storeModels)
            storeAmidstModels(models, "missing_results/continuous/"+ dataName + "/", dataNameMissing, "GLSL_"+this.initialization);

        /* Store experiment results in a JSON file */
        storeContinuousResults(scoresAndTimes, "missing_results/continuous/"+ dataName+"/",
                dataNameMissing + "_results_glsl_."+this.initialization+"json");
    }
    ...
    public void runMissingDiscrete(Tuple2<DataOnMemory<DataInstance>, List<DataOnMemory<DataInstance>>> runs,
        ...
        /* Store models */
        if(storeModels)
            storeAmidstModels(models, "missing_results/discrete/"+ dataName + "/", dataNameMissing, "GLSL");
    ...
    public void runMissingMixed(Tuple2<DataOnMemory<DataInstance>, List<DataOnMemory<DataInstance>>> runs,
        ...
        /* Store models */
        if(storeModels)
            storeAmidstModels(models, "missing_results/mixed/"+ dataName + "/", dataNameMissing, "GLSL_"+this.initialization);
